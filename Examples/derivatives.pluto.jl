...
begin
	...
	Pkg.Registry.add(RegistrySpec(url =
		"https://github.com/dojt/DOT_JuliaPackageRegistry.git"))
	Pkg.update()

	md"""
	A "package cell" that adds Dirk Oliver Theis's Julia Package Registry.  The names of the packages loaded from there have the prefix `DOT_`.
	"""
end

# ╔═╡ c711a86d-bdc3-4061-b911-cd0a494626b3
begin
...
Function
```
find_SR( Ω₀  ::Pulse__Ω_BangBang{ℚ,ℝ},
         hw ::HW_Descr{ℚ}            ) where{ℚ,ℝ}
```


1. ``K := t/2\pi`` -- but what does ``t`` refer to?
2. Get first rational approximation of ``2π\cdot (½)/2K``
3. ``\Omega_\text{amin}`` from HW description
3. ``\Omega_\text{max}, \Omega_\text{min}`` from pulse-shape description

"""
function find_SR( Ω₀  ::Pulse__Ω_BangBang{ℚ,ℝ},
				  hw  ::HW_Descr{ℚ}            ) where{ℚ,ℝ}
end
...
* 𝑡ᵒᶠᶠ:     $( @bind _Ω_tᵒᶠᶠ      Slider(0 : NoUnits(hw.𝑡ₘₐₓ/hw.𝑡ᵣₑₛ); default=100, max_steps=1+Int(NoUnits(hw.𝑡ₘₐₓ/hw.𝑡ᵣₑₛ)) ) )
...
f(;𝛺,𝛥) = let
	...
	evf(;𝛺, 𝛥,
	 	Ω_𝑡ᵒⁿ=3𝑡ᵣₑₛ, Ω_𝑡ᵒᶠᶠ=𝑡ᵒᶠᶠₘₐₓ,
	 	Δ_𝑡ᵒⁿ=3𝑡ᵣₑₛ, Δ_𝑡ᵒᶠᶠ=𝑡ᵒᶠᶠₘₐₓ,
	 	𝑇=𝑡ₘₐₓ,
	 	hw)
end
...
[[deps.DOT_NiceMath]]
deps = ["LinearAlgebra"]
git-tree-sha1 = "3ea290e2f61401e4e88067f79f89c5dec417c4d3"
uuid = "3e56a413-cb61-4004-9b36-fbc90ffc4c6e"
version = "0.1.5"

[[deps.DOT_RydSim]]
deps = ["DOT_NiceMath", "JSON", "LinearAlgebra", "Logging", "Unitful"]
git-tree-sha1 = "acfab6e3c000a87e9ebf0308da8221f87d2a6ae0"
